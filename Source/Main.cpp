...
HWND        hwndRenderFrame = NULL;           //渲染窗口句柄
HWND        hwndLogWindow   = NULL;
HWND        hwndLog         = NULL;
HINSTANCE   hinstMain       = NULL;          // 实例窗口
ConfigFile  *GlobalConfig   = NULL;             //系统配置全局变量
ConfigFile  *AppConfig      = NULL;             //profile 文件全局变量
OBS         *App            = NULL;             //全局配置对象
bool        bIsPortable     = false;              
bool        bStreamOnStart  = false;             //流是否启动
TCHAR       lpAppPath[MAX_PATH];                 //获取程序启动目录
TCHAR       lpAppDataPath[MAX_PATH];             //获取appdata 目录C:\Users\Administrator\AppData\Roaming\OBS
HDC         hMainBkground = NULL;               //框架背景
//----------------------------
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	//check  is support SSE2
    if (!OBSTools::HasSSE2Support())
    {
	...
	// first init 
    hinstMain = hInstance;
    // let developer know heap corruption as soon as 
    ...
    if(InitXT(NULL, TEXT("FastAlloc")))
    {
       ...
       if(lpAllocator)
        ...
        //-----------------------------------------------------
        // load locale  该变量在initXT中初始化，使用Str宏可以自动化解析
	   //首先加载english，后本地化语言
		...
		//DwmEnableComposition调用这个函数可关闭整个系统的玻璃效果
