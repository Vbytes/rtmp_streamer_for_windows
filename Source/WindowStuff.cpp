...
extern TCHAR   lpAppPath[MAX_PATH];                 //获取程序启动目录

void STDCALL SceneHotkey(DWORD hotkey, UPARAM param, bool bDown);
void SetBackground(HWND m_hWnd, const TCHAR* pBackImgFullPath,HDC hDC,BOOL bDrawEdage=TRUE,const TCHAR* content=NULL);
enum
{
    ID_LISTBOX_REMOVE = 1,
    ID_LISTBOX_MOVEUP,
...
LRESULT CALLBACK OBS::OBSStaticProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	     {
				...
				String bgpath;
				//bgpath << lpAppPath << "\\skin\\StaticSourceInfo.png";
				bgpath << lpAppPath << "\\skin\\staticbg.png";
				TCHAR szCaption[64] = {};
				GetWindowText(hwnd, szCaption, 63);
				SetBackground(hwnd, bgpath.Array(), hDC,FALSE,szCaption);
				EndPaint(hwnd, &ps);
	     }
		break;
	default:
...
LRESULT CALLBACK OBS::ListboxHook(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if (id == ID_SCENES)
	{
		...
		if (message == WM_ERASEBKGND)
		{
			hdc = GetDC(hwnd);
			String bgpath;
			bgpath << lpAppPath << "\\skin\\TAB.png";
			SetBackground(hwnd, bgpath.Array(), hdc, FALSE);
			return TRUE;
		}
	}
...
LRESULT CALLBACK OBS::OBSProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(message)
    {
		...
		case WM_MEASUREITEM://ODT_LISTBOX  
			if (((UINT)wParam == ID_SCENES) || ((UINT)wParam == ID_SOURCES))
			{
                        ...
                        if(nmh.code == NM_CUSTOMDRAW)
                        {
                            ...
                            switch( lplvcd->nmcd.dwDrawStage)
                            {
                                case CDDS_ITEMPREPAINT:
                                    ...
                                    {
										FillRect(hdc, &itemRect, (HBRUSH)GetStockObject(GRAY_BRUSH));
                                    ...
                                    {
										FillRect(hdc, &itemRect, (HBRUSH)GetStockObject(BLACK_BRUSH));
        ...
        case WM_DRAWITEM:
            if(wParam == ID_STATUS)
            {
            ...
            } else if (wParam==ID_SCENES) {
				//如果是场景控件
				LPDRAWITEMSTRUCT pDI = (LPDRAWITEMSTRUCT)lParam;
				HWND hSence = GetDlgItem(hwnd, ID_SCENES);

				HBRUSH brsh = NULL;
				if (pDI->itemState &ODS_SELECTED)
				{
					brsh = CreateSolidBrush(RGB(120, 120, 120));
					//设置字体颜色
					SetTextColor(pDI->hDC, RGB(0, 255, 0));
				}
				else
				{
					if (pDI->itemID % 2 == 0)
					{
						brsh = CreateSolidBrush(RGB(40, 40, 40));//yellow
					}
					else
					{
						brsh = CreateSolidBrush(RGB(10, 10, 10));
					}
					//设置字体颜色
					SetTextColor(pDI->hDC, RGB(136, 136, 136));
				}
				...
				SendMessage(hSence, LB_GETTEXT, pDI->itemID, (LPARAM)szText);
				const DWORD dwStyle = DT_CENTER | DT_SINGLELINE | DT_VCENTER | DT_NOPREFIX | DT_END_ELLIPSIS;
				
				DrawText(pDI->hDC, szText, _tcsclen(szText), &pDI->rcItem, dwStyle);
		...
		case WM_PAINT:
		     {
				  hdc = BeginPaint(hwnd, &ps);
				  String bgpath;
				  bgpath << lpAppPath << "\\skin\\Dialog_Default_back.png";
				  SetBackground(hwnd, bgpath.Array(),hdc);
				  EndPaint(hwnd, &ps);
				  break;
		      }
...
LRESULT CALLBACK OBS::ProjectorFrameProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_SETCURSOR:
        ...
        break;
	case  WM_PAINT:
		//SetBackground(hwnd, TEXT("d:\\test.jpg"));
		 break;
    default:
        return DefWindowProc(hwnd, message, wParam, lParam);
    }
...
LRESULT CALLBACK OBS::OBSStatuBarProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	//PAINTSTRUCT ps;
	//HDC         hdc;
	switch (message) {
	...
	case WM_DRAWITEM:
		...
		break;
	//case WM_SIZE:
	//	MoveWindow(hwnd, 0, 0, 1000, 100,true);
	//	break;
	//case WM_PAINT:
	//	{
	//		hdc = BeginPaint(hwnd, &ps);
...
void SetBackground(HWND m_hWnd, const TCHAR* pBackImgFullPath, HDC hDC, BOOL bDrawEdage, const TCHAR* content)
{
	...
	HBITMAP hbmpMem = CreateCompatibleBitmap(hDC, sizeWindow.cx, sizeWindow.cy);
	...
	if (bDrawEdage)
	{
		...
		Gdiplus::Pen pen(Gdiplus::Color(216, 92, 18), 5);
		graph.DrawRectangle(&pen, 0, 0, sizeWindow.cx, sizeWindow.cy);
		Gdiplus::Pen pen1(Gdiplus::Color(55, 55, 55), 2);
		Gdiplus::Point p1(sizeWindow.cx - 195, 5);
		Gdiplus::Point p2(sizeWindow.cx - 195, sizeWindow.cy - 5);
		graph.DrawLine(&pen1, p1, p2);
	}
	if (content)
	{

		Gdiplus::SolidBrush brush(Gdiplus::Color(200,200,200));
		graph.DrawString(content, _tcsclen(content), &Gdiplus::Font(TEXT("宋体"), 12, Gdiplus::FontStyleBold), Gdiplus::PointF(0, 2), &brush);
	}

	BitBlt(hDC, 0, 0, sizeWindow.cx, sizeWindow.cy, hdcMemory, 0, 0, SRCCOPY);
