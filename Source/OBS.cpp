...
const int totalBottomAreaHeight = 100;//170;//  控制面板的高度
const int totalHeaderAreaHegiht = 80;
const int TabControlAreaWidth = 200;//  选项卡得宽度
...
OBS::OBS()
{
   ...
   // wc.hbrBackground = GetSysColorBrush(COLOR_BTNFACE);
	...
	//-----------------------------------------------------
	////statu bar class
	//wc.lpszClassName = OBS_STATUS_CLASS;
	//wc.lpfnWndProc = (WNDPROC)OBS::OBSStatuBarProc;
	//wc.hIcon = LoadIcon(hinstMain, MAKEINTRESOURCE(IDI_ICON1));
	//wc.hbrBackground = (HBRUSH)(GetStockObject(GRAY_BRUSH));

	//if (!RegisterClass(&wc))
	//	CrashError(TEXT("Could not register status bar class"));
	//-----------------------------------------------------

    // main window class
    wc.lpszClassName = OBS_WINDOW_CLASS;
    wc.lpfnWndProc = (WNDPROC)OBSProc;
	//note:使背景为黑色的原因是避免原始背景和贴图背景差异大导致视觉上的闪烁
	...
	//该窗口不显示在状态栏WS_EX_TOOLWINDOW
	hwndMain = CreateWindowEx(0, OBS_WINDOW_CLASS, GetApplicationName(),
		WS_OVERLAPPED | WS_POPUP |WS_CLIPCHILDREN,
		x, y, defaultClientWidth, defaultClientHeight, NULL, NULL, hinstMain, NULL);
    ...
    HWND hwndTemp;
    hwndTemp = CreateWindowEx(0/*WS_EX_CLIENTEDGE,*/, TEXT("LISTBOX"), NULL,
		WS_CHILDWINDOW | WS_VISIBLE | WS_TABSTOP | LBS_HASSTRINGS | /*WS_VSCROLL |*/ LBS_NOTIFY | LBS_NOINTEGRALHEIGHT | WS_CLIPSIBLINGS | LBS_OWNERDRAWFIXED,
    ...
    hwndTemp = CreateWindowEx(0/*WS_EX_CLIENTEDGE*/, WC_LISTVIEW, NULL,
        WS_CHILDWINDOW|WS_VISIBLE|WS_TABSTOP|WS_VSCROLL|WS_CLIPSIBLINGS|LVS_REPORT|LVS_NOCOLUMNHEADER|
		LVS_SHOWSELALWAYS | LVS_ALIGNLEFT | LVS_NOLABELWRAP /*|LVS_OWNERDRAWFIXED*/,
    ...
    SetWindowLongPtr(hwndTemp, GWLP_WNDPROC, (LONG_PTR)OBS::ListboxHook);
	SendMessage(hwndTemp, LVM_SETBKCOLOR, 0, RGB(34,36,40));
 ...
 /*   hwndTemp = CreateWindow(TEXT("BUTTON"), Str("GlobalSources"),
    ...
    // scenes text
	  ...
	  hwndTemp = CreateWindow(TEXT("STATIC"), Str("MainWindow.Scenes"),
		  ...
		  SendMessage(hwndTemp, WM_SETFONT, (WPARAM)GetStockObject(DEFAULT_GUI_FONT), TRUE);
		 // SourceStaticProc = (WNDPROC)GetWindowLongPtr(hwndTemp, GWLP_WNDPROC);
		  SetWindowLongPtr(hwndTemp, GWLP_WNDPROC, (LONG_PTR)OBS::OBSStaticProc);
    //-----------------------------------------------------
    // source text
    ...
    if(numScenes)
    {
		...
		// over message to populate sources info
    ...
    //-----------------------------------------------------
    // Add built-in settings panes   初始化配置项对话框内容
	...
	if (bStreamOnStart){
		SendMessage(hwndMain, WM_COMMAND, MAKEWPARAM(ID_STARTSTOP, 0), NULL);
	} 
...
OBS::~OBS()
{
	...
	// modify by songmm on 2016-3-2 16:54
...
void OBS::ResizeRenderFrame(bool bRedrawRenderFrame)
{
	...
	if (bPanelVisible) {
		controlHeight -= totalHeaderAreaHegiht +totalBottomAreaHeight+ controlPadding;
		controlWidth-=TabControlAreaWidth+controlPadding;
	}
	...
	SetWindowPos(hwndRenderFrame, NULL, controlPadding, totalHeaderAreaHegiht + controlPadding, controlWidth, controlHeight, SWP_NOOWNERZORDER);
...
void OBS::ResizeWindow(bool bRedrawRenderFrame)
{
    ...
    int yStart = clientHeight - totalBottomAreaHeight;
    ...
    SetWindowPos(GetDlgItem(hwndMain, ID_MICVOLUME), NULL, 700,501,100,32, flags);
    xPos += controlWidth;

    SetWindowPos(GetDlgItem(hwndMain, ID_DESKTOPVOLUME), NULL, 700, 535, 100, 32, flags);
    xPos += controlWidth;
    ...
    ////SetWindowPos(GetDlgItem(hwndMain, ID_MICVOLUMEMETER), NULL, 200,53,100,10, flags);
    ////xPos += controlWidth;

    ////SetWindowPos(GetDlgItem(hwndMain, ID_DESKTOPVOLUMEMETER), NULL, 400,53,100,10,flags);
    //xPos += controlWidth;
	...
	SetWindowPos(GetDlgItem(hwndMain, ID_SCENES_TEXT), NULL, 800+7,3,191,25, flags);
    xPos += listControlWidth;

	SetWindowPos(GetDlgItem(hwndMain, ID_SOURCES_TEXT), NULL, 800+7,280,191,25, flags);
    xPos += listControlWidth;
	...
	SetWindowPos(GetDlgItem(hwndMain, ID_SCENES), NULL, 800 + 6/*xPos*/, 29/*yPos*/, 191, 250, flags);
    xPos += listControlWidth;

	SetWindowPos(GetDlgItem(hwndMain, ID_SOURCES), NULL, 800 + 6, 306, 191, 270, flags);
    xPos += listControlWidth;
}
...
void OBS::RefreshStreamButtons(bool disable)
{
	...
	btnPlayLive.EnableButton(!disable && canStream);
	btnRecord.EnableButton(!disable && canRecord);
	btnPreView.EnableButton(!disable && canTest);
...
void OBS::DrawStatusBar(DRAWITEMSTRUCT &dis)
{
    ...
    HBRUSH  hColorBrush = CreateSolidBrush(RGB(50,50,50));
    ...
    if(dis.itemID == 4)
    {
        ...
        rc.left += 22;
		SetTextColor(hdcTemp, RGB(0,255,0));
        String strKBPS;
...
{
	ResizeWindow(false);
	String strButtonPath;
	//--------------------------------------------------------------------
	//btnLogo
	const int logo_button_offset_left = 3;
	const int logo_button_offset_top = 3;
	const int logo_button_width = 200;
	const int logo_button_height = 80;
	RECT RT;
	RT.left = logo_button_offset_left;
	RT.top = logo_button_offset_top;
	RT.bottom = logo_button_offset_top + logo_button_height;
	RT.right = logo_button_offset_left + logo_button_width;
	strButtonPath << lpAppPath << "//skin//staticbk.png";
	btnLogo.Create(TEXT("月光石网络科技有限公司"), RT, hwndMain, ID_LOGO, strButtonPath.Array());
	btnLogo.EnableButton(TRUE);
	////--------------------------------------------------------------------
	//close  button
	const int close_button_offset_left = 776;
	const int close_button_offset_top = 3;
	const int close_button_width = 24;
	const int close_button_height = 24;
	//RECT RT;
	RT.left = close_button_offset_left;
	RT.top = close_button_offset_top;
	RT.bottom = close_button_offset_top + close_button_height;
	RT.right = close_button_offset_left + close_button_width;
	strButtonPath.Clear();
	strButtonPath << lpAppPath << "//skin//btn_login_close.png";
	btnClose.Create(TEXT(""), RT, hwndMain, ID_BTN_CLOSE, strButtonPath.Array());
	btnClose.EnableButton(TRUE);
	//--------------------------------------------------------------------
	//min button
	const int min_button_offset_left = 750;
	const int min_button_offset_top = 3;
	const int min_button_width = 24;
	const int min_button_height = 24;
	RT.left = min_button_offset_left;
	RT.top = min_button_offset_top;
	RT.bottom = min_button_offset_top + min_button_height;
	RT.right = min_button_offset_left + min_button_width;
	strButtonPath.Clear();
	strButtonPath << lpAppPath << "//skin//btn_login_min.png";
	btnMin.Create(TEXT(""), RT, hwndMain, ID_BTN_MINSIZE, strButtonPath.Array());
	btnMin.EnableButton(TRUE);
	//--------------------------------------------------------------------
	//btnPlugin
	const int plugin_button_offset_left = 760;
	const int plugin_button_offset_top = 40;
	const int plugin_button_width = 38;
	const int plugin_button_height = 38;
	RT.left = plugin_button_offset_left;
	RT.top = plugin_button_offset_top;
	RT.bottom = plugin_button_offset_top + plugin_button_height;
	RT.right = plugin_button_offset_left + plugin_button_width;
	strButtonPath.Clear();
	strButtonPath << lpAppPath << "//skin//btn_main_plugin.png";
	btnPlugin.Create(TEXT(""), RT, hwndMain, ID_PLUGINS, strButtonPath.Array());
	btnPlugin.EnableButton(TRUE);
	//--------------------------------------------------------------------
	//btnCarama
	const int carama_button_offset_left = 350;
	const int carama_button_offset_top = 10;
	const int carama_button_width = 72;
	const int carama_button_height = 72;
	RT.left = carama_button_offset_left;
	RT.top = carama_button_offset_top;
	RT.bottom = carama_button_offset_top + carama_button_height;
	RT.right = carama_button_offset_left + carama_button_width;
	strButtonPath.Clear();
	strButtonPath << lpAppPath << "//skin//btn_main_video.png";
	btnCarama.Create(TEXT(""), RT, hwndMain, ID_CARAMA, strButtonPath.Array());
	btnCarama.EnableButton(TRUE);
	//--------------------------------------------------------------------
	//btnSetting
	const int setting_button_offset_left = 250;
	const int setting_button_offset_top = 30;
	const int setting_button_width = 70;
	const int setting_button_height = 32;
	RT.left = setting_button_offset_left;
	RT.top = setting_button_offset_top;
	RT.bottom = setting_button_offset_top + setting_button_height;
	RT.right = setting_button_offset_left + setting_button_width;
	strButtonPath.Clear();
	strButtonPath << lpAppPath << "//skin//btn_main_sysSet.png";
	btnSetting.Create(TEXT("设置"), RT, hwndMain, ID_SETTINGS, strButtonPath.Array());
	btnSetting.EnableButton(TRUE);
	//--------------------------------------------------------------------
	//btnlive
	const int live_button_offset_left =400 ;
	const int live_button_offset_top = 520;
	const int live_button_width = 100;
	const int live_button_height = 32;
	RT.left = live_button_offset_left;
	RT.top = live_button_offset_top;
	RT.bottom = live_button_offset_top + live_button_height;
	RT.right = live_button_offset_left + live_button_width;
	strButtonPath.Clear();
	strButtonPath << lpAppPath << "//skin//btn_main_openLive.png";
	btnPlayLive.Create(Str("MainWindow.StartStream"), RT, hwndMain, ID_STARTSTOP, strButtonPath.Array());
	btnPlayLive.EnableButton(TRUE);
	////--------------------------------------------------------------------
	////btnTest
	//const int edit_button_offset_left = 150;
	//const int edit_button_offset_top = 520;
	//const int edit_button_width = 100;
	//const int edit_button_height = 32;
	//RT.left = edit_button_offset_left;
	//RT.top = edit_button_offset_top;
	//RT.bottom = edit_button_offset_top + edit_button_height;
	//RT.right = edit_button_offset_left + edit_button_width;
	//strButtonPath.Clear();
	//strButtonPath << lpAppPath << "//skin//btn_bk.png";
	//btnEdit.Create(TEXT("流编辑"), RT, hwndMain, ID_SCENEEDITOR, strButtonPath.Array());
	//btnEdit.EnableButton(TRUE);
	//--------------------------------------------------------------------
	//btnRecord
	const int record_button_offset_left = 280;
	const int record_button_offset_top = 520;
	const int record_button_width = 100;
	const int record_button_height = 32;
	RT.left = record_button_offset_left;
	RT.top = record_button_offset_top;
	RT.bottom = record_button_offset_top + record_button_height;
	RT.right = record_button_offset_left + record_button_width;
	strButtonPath.Clear();
	strButtonPath << lpAppPath << "//skin//btn_bk.png";
	btnRecord.Create(Str("MainWindow.StartRecording"), RT, hwndMain, ID_TOGGLERECORDING, strButtonPath.Array());
	btnRecord.EnableButton(TRUE);
	//--------------------------------------------------------------------
	//btnPreview
	const int preview_button_offset_left = 160;
	const int  preview_button_offset_top = 520;
	const int  preview_button_width = 100;
	const int  preview_button_height = 32;
	RT.left = preview_button_offset_left;
	RT.top = preview_button_offset_top;
	RT.bottom = preview_button_offset_top + preview_button_height;
	RT.right = preview_button_offset_left + preview_button_width;
	strButtonPath.Clear();
	strButtonPath << lpAppPath << "//skin//btn_bk.png";
	btnPreView.Create(Str("MainWindow.TestStream"), RT, hwndMain, ID_TESTSTREAM, strButtonPath.Array());
	btnPreView.EnableButton(TRUE);
	//--------------------------------------------------------------------
	//btnStop
	//const int  stop_button_offset_left = 530;
	//const int  stop_button_offset_top = 520;
	//const int  stop_button_width = 100;
	//const int  stop_button_height = 32;
	//RT.left = stop_button_offset_left;
	//RT.top = stop_button_offset_top;
	//RT.bottom = stop_button_offset_top + stop_button_height;
	//RT.right = stop_button_offset_left + stop_button_width;
	//strButtonPath.Clear();
	//strButtonPath << lpAppPath << "//skin//btn_bk.png";
	//btnStop.Create(TEXT("停止"), RT, hwndMain, ID_STOP, strButtonPath.Array());
	//btnStop.EnableButton(TRUE);

	ShowWindow(hwndMain, SW_SHOW);
}
void   OBS::AddVideoCaptureDevice()
{
	String strName;
	int i = 0;
	bool bFind = false;
	for (i = 0; i < imageSourceClasses.Num(); i++)
	{
		if (imageSourceClasses[i].strClass == String("DeviceCapture"))
		{
			strName =imageSourceClasses[i].strName;
			bFind = true;
			break;
		}
	...
	{
		HWND hwndSources = GetDlgItem(hwndMain, ID_SOURCES);
		XElement *curSceneElement =sceneElement;
		XElement *sources = curSceneElement->GetElement(TEXT("sources"));
		if (!sources)
		...
		newSourceElement->SetString(TEXT("class"), TEXT("DeviceCapture"));
		if (imageSourceClasses[i].configProc)
		{
			if (!imageSourceClasses[i].configProc(newSourceElement, true))
			{
				sources->RemoveElement(newSourceElement);
				EnableSceneSwitching(true);
				return;
			}
		}


		if (sceneElement == curSceneElement)
		{
			if (bRunning)
			{
				EnterSceneMutex();
				scene->InsertImageSource(0, newSourceElement);
				LeaveSceneMutex();
			}
			...
			InsertSourceItem(0, (LPWSTR)strName.Array(), true);
