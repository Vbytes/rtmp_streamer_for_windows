...
static const int defaultClientWidth  = 1000;
...
enum PacketType
{
    PacketType_VideoDisposable,//一次性的
    PacketType_VideoLow,
...
private:
	...
	COBSButton  btnClose;
	COBSButton  btnMin;
	COBSButton  btnPlugin;
	COBSButton  btnCarama;
	COBSButton  btnSetting;
	COBSButton  btnPlayLive;
	/*COBSButton  btnEdit;*/
	COBSButton  btnRecord;
	COBSButton  btnPreView;         //流预览按钮
	//COBSButton  btnStop;
	//---------------------------------------------------
	// mainly manly main window stuff
    String  strLanguage;
    bool    bTestStream;//是否在预览流
  ...
  //  HWND    hwndRenderMessage; // The text in the middle of the main window
    ...
    List<ClassInfo> imageSourceClasses;   //包含所有的源信息
    ...
    bool bShuttingDown;   //为加速键线程设计的变量，未初始化应该值为0
	...
	static LRESULT CALLBACK OBSStatuBarProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam);
	static LRESULT CALLBACK OBSStaticProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam);
