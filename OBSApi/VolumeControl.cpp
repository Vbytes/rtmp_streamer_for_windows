...
void VolumeControlData::DrawVolumeControl(HDC hDC)
{
	...
	HBRUSH hBg = CreateSolidBrush(RGB(36,36,36));
    FillRect(hdcTemp, &clientRect, hBg);

    HBRUSH hGray = CreateSolidBrush(RGB(20,20,20));
    HBRUSH hGreen  = CreateSolidBrush(RGB(0,180,0));//0xff4040);
    ...
    if(volSliceSize > 1.0f)
    {
        ...
        for(int i=0; i<10; i++)
        {
            ...
            if(sliceRect.right < volPixelPos) //full
				FillRect(hdcTemp, &sliceRect, hGreen);
            else if(sliceRect.left < volPixelPos) //half
            {
				...
				FillRect(hdcTemp, &leftHalf, hGreen);
